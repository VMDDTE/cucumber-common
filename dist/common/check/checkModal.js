"use strict";

/**
 * Check if a modal was opened
 * @param  {String}   modalType  The type of modal that is expected (alertbox,
 *                               confirmbox or prompt)
 * @param  {String}   falseState Whether to check if the modal was opened or not
 */
module.exports = function (modalType, falseState) {
  /**
   * The text of the prompt
   * @type {String}
   */
  var promptText = '';

  try {
    promptText = browser.alertText();

    if (falseState) {
      expect(promptText).to.not.equal(null, "A ".concat(modalType, " was opened when it shouldn't"));
    }
  } catch (e) {
    if (!falseState) {
      expect(promptText).to.equal(null, "A ".concat(modalType, " was not opened when it should have been"));
    }
  }
};