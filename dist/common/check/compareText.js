...
module.exports = function (element1, falseCase, element2) {
  ...
  if (falseCase) {
    expect(text1).to.not.equal(text2, "Expected text not to be \"".concat(text1, "\""));
  } else {
    expect(text1).to.equal(text2, "Expected text to be \"".concat(text1, "\" but found \"").concat(text2, "\""));
  }
};